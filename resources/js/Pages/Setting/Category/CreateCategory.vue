<template>
    <modal :show="ShowingModal" @close="closeModal">
        <div class="relative p-4">
            <div v-if="$page.props.flash.successTwo" class="bg-zinc-600 text-white rounded text-sm font-semibold absolute top-0 right-2/4 p-2 translate-x-2/4"> {{ $page.props.flash.successTwo }} </div>
            ...
            <div class="mb-4">
                ...
                <div v-if="categoryError" class="text-sm font-semibold text-white bg-rose-500 p-2 rounded absolute top-0 right-2/4 translate-x-2/4">
                    {{ categoryError }}
                </div>
            </div>
            ...
            <div v-if="created">
                ...
                <div :class="{'mb-4': products.length > 0}">
                    <div v-for="(product, index) in products" :key="product.id" class="flex gap-4 items-center  px-4 py-2 hover:bg-zinc-100">
                        ...
                        <div class="w-10 h-10 overflow-hidden relative rounded">
                            <img :src="'/images/phones/' + product.avatar" class="w-full absolute left-0 top-2/4 -translate-y-2/4">
                        </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <label for="avatar" v-show="avatar == null" class="font-medium text-sm text-gray-700 cursor-pointer">{{__('Select Image')}}</label>
                ...
                <div v-if="productError" class="text-sm font-semibold text-white bg-rose-500 p-2 rounded absolute bottom-0 right-2/4 translate-x-2/4">
                    {{ productError }}
                </div>
            </div>
