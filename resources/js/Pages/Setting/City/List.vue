<template>
    <div class="relative rounded sm:rounded-lg shadow sm:shadow-xl">
        ...
        <button @click.prevent="ShowingModal = true" class="absolute -top-12 right-2 inline-flex items-center justify-center gap-2 py-2 px-4 text-xs font-bold tracking-wide uppercase hover:bg-zinc-950 bg-transparent text-zinc-950 hover:text-zinc-50 transition-all easy-in-out rounded border border-zinc-950">
            <span class="material-icons-outlined min-w-min">playlist_add</span>
            {{ __('New City') }}
        </button>

        <div v-if="$page.props.flash.successTwo" class="text-white font-semibold text-sm bg-zinc-600 p-2 rounded absolute right-0 bottom-2/4 translate-y-2/4 z-10"> 
        ...
        <!-- content -->
        <div v-for="(city, index) in cities" :key="city.id"   class="grid grid-cols-5 items-center px-4 py-2 hover:bg-zinc-100 " >
    ...
    <Modal :show="ShowingModal" @close="closeModal">
        <div class="p-4" >
            <div v-if="$page.props.flash.successThree" class="bg-zinc-600 text-white rounded text-sm opacity-80 w-8/12 text-center py-2 font-semibold absolute top-0 right-2/4 p-2 translate-x-2/4"> {{ $page.props.flash.successThree }} </div>
            ...
            <div class="mb-4">
                ...
                <div v-if="cityError" class="text-sm font-semibold opacity-80 text-white bg-rose-500 p-2 rounded absolute top-0 right-2/4 translate-x-2/4">
                    {{ cityError }}
                </div>
            </div>
        </div>
