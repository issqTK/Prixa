<template>
    <div>
        <h3 class="text-lg uppercase text-zinc-700 font-bold mb-3">{{__('Logo Setting')}}</h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4">
            <div class="group relative pt-6 p-4 bg-white shadow rounded overflow-hidden">
                <h4
                    class="absolute top-0 left-2/4 -translate-x-2/4 w-full text-sm font-semibold text-zinc-900 bg-zinc-200 rounded-t text-center">
                    {{ __('Front Office') }}
                </h4>

                <img :src="this.frontLogo == null
                    ? '/images/logos/' + this.logos.frontLogo
                    : this.frontLogo" draggable="false">

                <input id="front" type="file" class="hidden" @change="uploadLogoFront($event)">

                <label for="front"
                    class="absolute left-0 right-0 h-full -top-full group-hover:top-0  font-semibold text-zinc-50 flex items-center justify-center cursor-pointer transition-all"
                    style="background-color: #18181b16; backdrop-filter: blur(10px);">
                    {{ __('Upload New') }}
                </label>
            </div>
            ...
            <div class="group relative pt-6 p-4 bg-white shadow rounded overflow-hidden">
                ...
                <img :src="'/images/logos/' + this.logos.backLogo" draggable="false" class="">
            ...
            <div class="group relative pt-7 p-4 bg-white shadow rounded overflow-hidden">
                <h4
                    class="absolute -top-0 left-2/4 -translate-x-2/4 w-full text-sm font-semibold text-zinc-900 bg-zinc-200 rounded-t text-center">
                    {{ __('Favicon') }}
                </h4>

                <img :src="this.faviconLogo == null
                    ? '/images/logos/' + this.logos.faviconLogo
                    : this.faviconLogo" draggable="false"
                    class="mx-auto w-5/12 "
                    >
